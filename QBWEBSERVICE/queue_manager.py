...
class RabbitMQManager(QueueManager):
    ...
    def _get_connection(self) -> BlockingConnection:
        ...
        self._connection = BlockingConnection(
            ConnectionParameters(
                ...
                port=qbwc_settings.RABBITMQ_DEFAULT_PORT,
    ...
    def publish_message(self, msg, queue_name, delete_queue=False):
        ...
        channel.queue_declare(queue_name, arguments={
                              'x-queue-mode': 'lazy', 'x-max-length': 1000})
    ...
    def get_queue_message_count(self, queue_name):
        print("get_queue message count is called")
        message_count = self._get_channel().queue_declare(
            queue=queue_name,
